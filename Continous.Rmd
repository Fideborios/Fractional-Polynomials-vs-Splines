...
m1 <- bam(dif_liver_perc ~ Drug * Age*Gender + s(Age)  + s(Study,Drug,Gender, bs = "re") , data = somatostatin,
...
# Note: this is really not the best fitting model for the data:
m1 <- bam(poutcome ~ bilat_0 * treat*age + s(age)  + s(study,treat,age, bs = "re"), data=IPDMA, discrete=TRUE, family = binomial("log"))
...
# Note: this is really not the best fitting model for the data:
m1 <- gamm(poutcome ~ bilat_0*treat*age, random = list(study = ~1 + age + treat + bilat_0) , data=IPDMA, discrete=TRUE, family = binomial("log"))



get_difference(model = m1$gam , comp=list(treat=c("Placebo", "Antibiotics"))   )
