...
For the Gevers et al. data-analysis, we assumed a linear functional form, as we had limited data (108 observations) and spending $2 \times p$ (degree of polynomial) $\times  J$ (trial number) was considered unefficient. Furthemore, the initial pooled plot showed no significant non-linearity. In contrast, for the Rovers et al. we observed an overall quadratic shape. Damoiseaux et al. had age rounded and the participants were only 1 and 2 years old children. To avoid non-convergance of the log-binomial models we created a slight artificial deviation using the **jitter()** command.
...
ymat <- matrix(NA,length(unique(IPDMA$study)),
               6,dimnames=list(unique(IPDMA$study),c("Intercept","Treatment","Age","Age^2","Treatment x Age","Treatment x Age^2")))
...
for(i in unique(IPDMA$study)) {
    ...
    # RUN THE MODEL
    model <- glm(poutcome ~ treat * age + treat *Agesq  , 
                 family = binomial("log"), data)
