...
*Two-stage Global polynomials*


For the Gevers et al. data-analysis, we assumed a linear functional form, as we had limited data (108 observations) and spending $2 \times p$ (degree of polynomial) per trial j was considered unefficient. Furthemore, the initial pooled plot showed no significant non-linearity. In contrast, for the Rovers et al. we observed a quadratic shape and used it as prior knowledge.
...
For the Gevers et al the two-stage meta-analysis of interaction terms showed no statistically significant. 

```{r drawing the continuous outcome plot}

df = IPDMA%>%
  group_by(study)%>%
  do(fit = glm(poutcome~ treat *age + treat*I(age^2), data = .))
...
df_two_stage%>%
  gather(study, estimate)
  

###
Interactions1 =  df_two_stage[which(df_two_stage$term == "treatAntibiotics:age"),]
Interactions2 =  df_two_stage[which(df_two_stage$term == "treatAntibiotics:I(age^2)"),]


model <- mvmeta(cbind(Interactions1,Interactions2),S,data=hsls,method="mm")
summary(model)


par(mfrow= c(1,2))
forest(rma(data = Interactions1, yi = estimate, sei = std.error, slab = study) )
forest(rma(data = Interactions2, yi = estimate, sei = std.error, slab = study) )


```

While for the Rovers et al we 
...
glmLinear <- glmer(poutcome ~ treat * age*bilateral + (treat|study) + (1|study) , family = binomial("log"),data = IPDMA )
```
