...
ggscatter(data = IPDMA, x = "age",y =  "poutcome",color = "treat",ggtheme = theme_minimal,
          add = "loess",palette = "lancet", conf.int = T, legend.title = "Treatment", conf.int.level = 0.835,
...
for(i in studies){
    ...
    # RUN THE MODEL
    model <- bam(poutcome ~ treat*bilat_0 + s(age) + s(age,by = treat) + s(age,by = bilat_0), 
...
For the liver data set the two-stage meta-analysis of interaction terms showed no statistically significant results. The reason we the same as in splines and FPs. The per trial

`r kable(ymat, format = "latex", booktabs = T)%>%
kable_styling(latex_options = c("striped", "scale_down"),full_width = F)%>%
column_spec(1, bold = T, width = "10em") %>%
column_spec(2, width = "10em")%>%
column_spec(3, width = "6em")%>%
column_spec(4, width = "10em")` coefficients 
...
glmLinear <- glmer(poutcome ~ treat *age*bilateral + (treat|study) + (1|study) , family = binomial("logit"),data = IPDMA )
```
...
glm.effect.boundaries =  data.frame(effect(c("age","treat","bilateral"),glmLinear))
