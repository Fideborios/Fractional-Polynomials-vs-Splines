...
```{r Script for Basis splines (cubic) Heterogeneous Data }
...
new.data = new.data[which(max(minidf$BMI )>new.data$BMI & new.data$BMI  >= min(minidf$BMI )),]
...
predicted.values[which(max(minidf$BMI )>predicted.values$BMI & predicted.values$BMI  >= min(minidf$BMI )),k] =  temp.pred.df[,1]
predicted.values[which(max(minidf$BMI )>predicted.values$BMI & predicted.values$BMI  >= min(minidf$BMI )),k+1] =  temp.pred.df[,2]
...
ggplot(long.predicted.values,aes(x = BMI, probability, linetype= Treatment, color= Treatment)) + 
  geom_line() + facet_wrap(Study~., ncol = 2, dir="v")+ 
  ylab("Estimated Mortality risk") + scale_color_jama()+ 
  labs(title = "Cubic basis splines with 6 knots in IPD with different renages",
       subtitle = "3 evenly placed knots were placed per study" , 
       caption = "Figure 4.") + 
  theme_bw()+ geom_ribbon(mapping = aes(ymin=Lower, ymax=Upper),alpha=0.25)+
  theme(plot.title    = element_text(hjust = 0.5,size = 32,face = "bold.italic"),
        plot.subtitle = element_text(hjust = 0.5,size = 24,face = "bold.italic"),
...
newdata = data.frame(df[,c("Study","BMI","Treatment")],dum=rep(0,dim(df)[1]))
preds=  predict.gam(fit, newdata = newdata,se.fit = T)
